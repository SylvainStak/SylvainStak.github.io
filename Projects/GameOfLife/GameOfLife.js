$(document).ready(() => {
  ...
  document.getElementById('hide_menu').addEventListener('click', async () => {
    document.getElementById('menu').style.right = -400;  
...
var tileWidth, tileHeight, nCols, nRows, grid, bounds, showGrid, drawing;

function setup() {
  field = createCanvas(windowWidth, windowHeight);
...
function calculateNextGeneration() {
  ...
  for (let i = offset; i < nCols-offset; i++)
    for (let j = offset; j < nRows-offset; j++) {
      ...
      if ((neighbors < 2 || neighbors > 3) && actualCell === 1) nextGrid[i][j] = 0; //Dead Cell
      else if (neighbors === 3 && actualCell === 0) nextGrid[i][j] = 1; //Alive Cell
